...
최근 급증하는 지능형 스미싱(택배 사칭, 정부지원금 등) 문제를 해결하기 위해, 단순 키워드 매칭을 넘어 **Upstage Solar LLM**의 문맥 이해 능력을 활용한 실시간 분석 및 대용량 데이터 기반 대시보드를 제공합니다.
...
| **민채영** | AI / Logic | 분석 로직 개발, 추출 데이터 DB 적재 프로세스 설계 |
...
- **Score Gauge**: 0~100% 사이의 위험도를 직관적인 게이지 차트로 표현
...
- **문제**: 기존 데이터는 단순 텍스트 형식이라 실시간 AI 분석 데이터와 규격이 맞지 않음.
- **해결**: 
